...
const useApi = () => {
  const { web } = useSelector((state: { user: UserState }) => state.user);

  console.log('TOKEN:', web);
  ...
  const getSingleTransactionOfAUserForAdmin = async (
  ...
  const imageProfileUpload = async (formData: object) => {
    try {
      const result = await axios.post(ImageUploadRoute, formData, {
        headers: {
          ...authHeader,
